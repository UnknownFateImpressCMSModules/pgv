...
/*=================================================
	...
	Just put this in your phpGedView folder. It is called by pgvindex.php which should be (with post-config.php and postwrap.js in modules/phpGedView)
...
// get the phpGedView config - our config comes through via cookies
		...
		if (isset($_COOKIE['post_fullname'])) $post_fullname = $_COOKIE['post_fullname'];
		...
		if (!empty($post_user)) {
			...
			if (!getUser($post_user) and ($def_create_user == "yes") )
			{
				...
				$newuser["fullname"]=$post_fullname;
			...
			if (getUser($post_user) and ($admin_ver == "yes"))
			{
				...
				{
					$users[$post_user]["loggedin"]="Y";
			        $users[$post_user]["sessiontime"]=time();
			        storeUsers();
					AddToLog("Login Successful ->" . $post_user ."<-");
				}
